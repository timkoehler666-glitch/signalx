...
export async function fetchYahooMarketSnapshot(ticker:string):Promise<MarketSnapshot>{
  ...
  const summaryUrl=new URL(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(symbol)}`);summaryUrl.search=new URLSearchParams({modules:"summaryDetail,defaultKeyStatistics,calendarEvents,assetProfile"}).toString();
  ...
  const sector=details?.assetProfile?.sector??null,industry=details?.assetProfile?.industry??null;
  return{provider:"yahoo",providerAttempts:["yahoo"],providerChecks:[],symbol:result.meta.symbol,name:result.meta.longName??result.meta.shortName??null,sector,industry,stockCategory:classifyStock(sector,industry),exchange:result.meta.exchangeName??null,currency:result.meta.currency??null,price,change,changePercent:change!=null&&previous?change/previous*100:null,asOf:result.meta.regularMarketTime?new Date(result.meta.regularMarketTime*1000).toISOString():chart.at(-1)?.date??null,trailingPe:trailingPe??null,forwardPe:details?.summaryDetail?.forwardPE?.raw??details?.defaultKeyStatistics?.forwardPE?.raw??null,dividendYieldPercent:details?.summaryDetail?.dividendYield?.raw!=null?details.summaryDetail.dividendYield.raw*100:null,nextDividendDate:dividendDate?new Date(dividendDate*1000).toISOString().slice(0,10):null,chart,warnings};
}
